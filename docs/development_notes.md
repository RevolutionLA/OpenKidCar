...
- 小车端 **干杯助手语音对话**：`📡 干杯助手` 按钮按住说话 → 桥接 `xiaozhi_bridge.py`(8010) → 小智官方服务器 → 语音回复流式播放（双进程架构）
- **干杯助手语音控制小车**：说"打开大灯/灯带/静音/档位/急刹"→ MCP 工具 → `/api/control` → 大脑 → 小车执行
- 家长端（`:8001`，手机一屏）：车辆状态、GPS 实时轨迹地图、电量监控、视频通话（按需显示窗格）、对讲 PTT、远程急刹
- 真实音效：引擎（CC0 engine.mp3，随油门变速）+ 喇叭（CC BY 汽车鸣笛）；单向视频双向语音
